"""
Analytics script: Calculate top 5 clients by revenue using DuckDB on Curated Layer.
...
def calculate_top_clients():
    """
    Calculate top 5 clients by revenue from Curated Layer using DuckDB.
    Results are saved to a Parquet file in the analytics layer.
    """
    print("Calculating top 5 clients by revenue from Curated Layer...")
    
    # Read SQL query from file
    sql_file = Path(__file__).parent.parent / "sql" / "analytics" / "top_clientes.sql"
    ...
    print("Top 5 Clients Results:")
    print(df.to_string(index=False))
    
    # Save to analytics layer
    output_file = ANALYTICS_DATA_DIR / "top_clientes.parquet"
