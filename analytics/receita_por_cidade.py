"""
Analytics script: Calculate revenue by city using DuckDB on Curated Layer.
...
def calculate_revenue_by_city():
    """
    Calculate revenue by city from Curated Layer using DuckDB.
    Results are saved to a Parquet file in the analytics layer.
    """
    print("Calculating revenue by city from Curated Layer...")
    
    # Read SQL query from file
    sql_file = Path(__file__).parent.parent / "sql" / "analytics" / "receita_por_cidade.sql"
    ...
    print("Revenue by City Results:")
    print(df.to_string(index=False))
    
    # Save to analytics layer
    output_file = ANALYTICS_DATA_DIR / "receita_por_cidade.parquet"
